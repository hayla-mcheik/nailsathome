...
const props = defineProps({ turnbacktime: Array });
...
const submit = () => {
    if (isEditing.value) {
        // Correct way to spoof PUT for file uploads in Inertia
        form.transform((data) => ({
            ...data,
            _method: 'put',
        })).post(route('admin.turnbacktime.update', editId.value), {
            onSuccess: () => resetForm(),
        });
    } else {
        form.post(route('admin.turnbacktime.store'), {
            onSuccess: () => resetForm(),
        });
    }
};
...
const deleteItem = (id) => {
    if (confirm('Delete this luxury treatment?')) {
        router.delete(route('admin.turnbacktime.destroy', id));
    }
};
</script>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-1">
                    <div class="bg-white p-6 rounded-lg shadow border sticky top-24">
                        <h3 class="text-lg font-bold mb-4">{{ isEditing ? 'Edit' : 'Add' }}Let's Turn Back Time</h3>
                ...
                <div class="lg:col-span-2">
                    <div class="space-y-4">
                        <div v-for="item in turnbacktime" :key="item.id" class="bg-white p-4 rounded-lg shadow border flex gap-4">
